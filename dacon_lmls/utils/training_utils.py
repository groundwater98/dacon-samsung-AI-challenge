...
from transformers import (
    PreTrainedModel, PreTrainedTokenizerFast, PreTrainedTokenizer, SwitchTransformersForConditionalGeneration
)
...
def accelerate_run_train(
    ...
    no_eval_until_steps = training_args.no_eval_until_epochs * num_update_steps_per_epoch
    ...
    for epoch in range(training_args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.is_local_main_process and completed_steps % training_args.log_steps == 0:
            ...
            if accelerator.sync_gradients:
                ...
                completed_steps += 1
            if completed_steps % training_args.checkpoint_steps == 0 and completed_steps > 0:
            ...
            if (completed_steps % training_args.num_eval_steps == 0 and
                completed_steps >= no_eval_until_steps and accelerator.sync_gradients) or (
                    step == 0 and training_args.eval_at_the_beginning
            ):
                if eval_dataloader is not None:
                    ...
                    for eval_step, eval_batch in enumerate(eval_dataloader):
                        ...
                        if evaluate_fn is not None:
                            ...
                            output_labels += torch.cat([
                                eval_labels,
                                torch.ones(eval_labels.shape[0],
                                           tokenizer.model_max_length - eval_labels.shape[1],
                    ...
                    if evaluate_fn is not None:
                        ...
                        eval_res = evaluate_fn(predictions=output_predictions, labels=output_labels)
                    else:
                        eval_res = dict()
                    metric_key = list(eval_res.keys())[0] if len(eval_res) > 0 else evaluate_loss_key
                    ...
                    if (is_language_modeling and eval_res[metric_key] < best_eval) or (
                        ...
                        best_eval = eval_res[metric_key] if is_language_modeling else -eval_res[metric_key]
                        wandb.summary["best_" + metric_key] = eval_res[metric_key]
                        accelerator.wait_for_everyone()
                        unwrapped_model = accelerator.unwrap_model(model)
                        unwrapped_model.save_pretrained(os.path.join(training_args.output_dir, "best"),
                                                        is_main_process=accelerator.is_local_main_process,
                                                        save_function=accelerator.save)
    ...
    accelerator.wait_for_everyone()
    unwrapped_model = accelerator.unwrap_model(model)
    unwrapped_model.save_pretrained(os.path.join(training_args.output_dir, "checkpoint-{}".format(completed_steps)),
                                    is_main_process=accelerator.is_local_main_process,
                                    save_function=accelerator.save)
    tokenizer.save_pretrained(os.path.join(training_args.output_dir, "checkpoint-{}".format(completed_steps)))
